...
public class MappingCost {
	...
	public static double calculate(Vector<Mapping> map) {
		...
		for (int agentNum = 0; agentNum < map.size(); agentNum++) {
			...
			cost += TriangleLocalization.FindDistanceAmong2Coordinates(NewAlgorithm.agentPosition[map.elementAt(agentNum).getAgent()-2],NewAlgorithm.Position[map.elementAt(agentNum).getPosition()-2]);
...
//
//
//					Coordinate Agent1 = NewAlgorithm.agentPosition[map.elementAt(q).getAgent()-1];
//
//					Coordinate Agent2 = NewAlgorithm.agentPosition[map.elementAt(q).getAgent()-1];
//
...
//								NewAlgorithm.Position[map.elementAt(q).getAgent()-1], Agent2, NewAlgorithm.Position[map.elementAt(r).getAgent()-1]);
//
//						cost -= distance;
//
//					}
