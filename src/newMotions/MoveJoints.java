	...
	public  static String performMove(Phase pha){
		...
		if(pha.movements.size()>0){
			for(int i=0;i<pha.movements.size();i++){
				...
				if(WalkLeaning.getLean().equalsIgnoreCase("left")||WalkLeaning.getLean().equalsIgnoreCase("right")){

					
					Float move=pha.movements.elementAt(i).value;
					
					
					if(WalkLeaning.getLean().equalsIgnoreCase("left")&&MotionPlaying.getMotionName().equalsIgnoreCase("walk_fine")&&pha.getName().equalsIgnoreCase("walk_attempt4")){
						if(joint.equalsIgnoreCase("lle3")){
							move=(float) 53;
						}else if(joint.equalsIgnoreCase("lle5")){
							move=(float) 32;
						}
					}else if(WalkLeaning.getLean().equalsIgnoreCase("right")&&MotionPlaying.getMotionName().equalsIgnoreCase("walk_fine")&&pha.getName().equalsIgnoreCase("walk_attempt6")){
